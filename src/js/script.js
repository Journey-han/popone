...
function renderResults() {
    ...
    */
    ...
    function generateTickValues(newCurrentValue) {
        const step = 10; // 눈금 간격
        const rangeSize = 100; // 구간의 크기
        const minTickValue = Math.floor(newCurrentValue / rangeSize) * rangeSize; // 구간의 최소값
        const maxTickValue = minTickValue + rangeSize; // 구간의 최대값
    
        // 최소값에서 최대값까지 step 간격으로 배열을 생성
        let tickValues = [];
        for (let i = minTickValue; i <= maxTickValue; i += step) {
            tickValues.push(i);
        }
    
        // 전역 변수 newTickValues를 업데이트
        newTickValues = tickValues;
        return newTickValues;
    }

    console.log(newTickValues);
